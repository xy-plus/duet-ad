...
def test_prompt_fusion_sends_exact_hard_cut_visuals_per_segment(
    tmp_path: Path,
) -> None:
    digest = "d" * 64
    schema = codex_output_schemas.prompt_fusion_schema(
        input_sha256=digest, visual_counts=(1, 2), visual_max_chars=426,
    )
    result = {
        ...
        "input_sha256": digest,
        "segments": [
            {"index": 1, "visual": ["shot one"]},
            {"index": 2, "visual": ["shot one", "shot two"]},
        ],
    }
