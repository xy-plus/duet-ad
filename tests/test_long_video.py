...
def test_provider_integer_duration_boundaries_do_not_hide_positive_overflow():
    ...
    segments = plan_segments(10.000001, [(0.0, 10.000001)], [])
...
def test_long_scene_is_split_at_production_limit_and_continues_chain():
...
def test_every_new_plan_obeys_the_ten_second_production_contract(duration):
    ...
    assert all(
        provider_duration_s(segment["start_s"], segment["end_s"])
        <= long_video_module.SEGMENT_PROVIDER_MAX_DURATION_S
        for segment in segments
    )
    assert (len(segments) == 1) == (
        duration <= long_video_module.SHORT_VIDEO_MAX_S
    )
...
def test_real_scene_bounds_never_plan_over_production_limit():
    duration = 50.64
    ends = [
...
def test_local_dialogue_is_shifted_clipped_and_owned_once():
    lines = [
    ...
    plan = [
        ...
        {"index": 2, "start_s": 10.0, "end_s": 15.0},
    ]
    assert localize_dialogue(lines, plan[1], segments=plan) == [
        {"text": "a", "start_s": 0.0, "end_s": 1.25},
        {"text": "b", "start_s": 4.5, "end_s": 5.0},
    ]

    crossing = [{"text": "crosses", "start_s": 9.5, "end_s": 10.5}]
    fragments = [
        localize_dialogue(crossing, segment, segments=plan)
        for segment in plan
    ]
    assert fragments == [
        [{"text": "cros", "start_s": 9.5, "end_s": 10.0}],
        [{"text": "ses", "start_s": 0.0, "end_s": 0.5}],
    ]
    assert "".join(
        item["text"] for items in fragments for item in items
    ) == crossing[0]["text"]
