...
def _source_request(tmp_path: Path) -> tuple[h3.H3Request, dict]:
    ...
    on_screen = context_ir_bridge.format_speech_marker(
        order=1,
        delivery="on_screen",
        subject_id="S1",
        voice_ref=1,
    ...
    off_screen = context_ir_bridge.format_speech_marker(
        order=2,
        delivery="off_screen",
        subject_id=None,
        voice_ref=1,
    ...
    prompt = request.prompt.replace(
        "[1] <Subject 1>(S1) says exactly <d>[Chinese]我会准时回来。</d>.",
        on_screen,
    ).replace(
        "[2] The off-screen narrator using <Audio 1> says exactly "
        "<d>[Chinese]随后转为画外旁白。</d>.",
        off_screen,
    )
