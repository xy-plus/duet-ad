...
每个 hard-cut 区间先建立内部证据账本（不输出）：逐帧记录当前帧直接可见的 `stable_key`/`TILE_XX` 及可见范围；仅消费同一当前帧同时映射且可见的绑定，禁止用不属于当前区间的前一帧、相邻区间或其他 segment 的人物、服装、环境、对象、动作补当前区间。边缘、局部、遮挡、模糊或紧裁切只能写可见片段；动态须有区间起止帧证据并止于末帧。末帧若仍见运动/残留，只写该可见状态；完成、收回、停住、稳定或 release/stop 等终态，须由末帧直接可见且前帧支持，禁止旧提示词补出。`audio_content` 仅作为冻结台词事件的冲突边界，不生成声音、拟声、台词、说话动作或口型。无证据短语删除，不拒绝、不重试、不回退、不改 schema。
