...
逐段输出只有 9 张关键帧和旧视频提示词。完整写入目标文件，`prompt.txt` 最后写入并立即退出；校验和正式发布由后端负责。
...
`replaceable` 只写可替换属性，`preserve` 只写必须保持的身份、形态、关系或连续性。空类别写 `{}`。将完整、可解析的 JSON 写入目标后立即退出；校验和正式发布由后端负责。
