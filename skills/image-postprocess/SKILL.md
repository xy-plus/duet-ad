...
段级提示词会复用于该段每次以一张当前源图为编辑目标的请求；每段所有关键帧共享一份可直接提交给 Seedream 的提示词。第一句只聚焦唯一替换目标、冻结的自然语言身份短语及其原位条件；第二句写完整保护。两句只写对每次当前待编辑源图都成立的通用投影守恒，不得逐帧描述条件，不得声称同时看过多张输入图。保持简洁，但不得为缩短删除安全关系；禁止全景复述、画质美化或实体清单，不得包含 Markdown、解释或分析。
