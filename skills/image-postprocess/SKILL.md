...
最终视频连续性与现实合理性是最高优先级。人物与真实新场景必须同时替换并形成明显表象差异，同时保持可见叙事事实、动作目的、相机和全局光色不变；两者冲突时判项目不合格，不得牺牲现实关系换取替换幅度，也不得输出成功 no-op。
...
1. 查看全项目全部冻结帧，先建立镜头、场景组件、人物轨道和可见关系图；逐帧记录交互、接触、持握、支撑、遮挡、前后顺序、视线、姿态、动作目的、数量与尺度。`continue` 优先视为连续画面。`hard_cut` 是场景证据边界：不得把切前场景传播到切后；切后场景组件只能依据切后可见证据独立判定。只有切后证据自身足以确认同一物理环境时，才可重新绑定同一场景包。
2. 找出全部叙事主人物。叙事主人物是持续承担动作、对白、演示或剧情作用的人；背景路人不是替换目标，写入 `protected_non_target_people`。不能可靠区分多位主人物或可能串人时，保守判不合格，不得错误合并。
3. 为每个稳定叙事主人物建立一个 `person_plans` 项，作为冻结新身份包；同一人物全部 `replace` 段逐段复用同一身份、服装设计、局部固有色和 reference，不得逐帧重新设计，也不得从编辑结果反推下一帧。替换人物保持源人物可见的性别呈现、肤色与族裔外观风格范围、年龄范围和整体气质，但不推断或命名敏感身份；使用长相略有不同的新脸。服装保持用途、颜色关系和整体风格，但款式与局部主色必须产生可见差异。
4. 每段 `persons` 必须按 ID 完整枚举所有主人物。人物在本段任一帧可观察且边界可定位时写 `replace`，并列出所有可观察帧；确实不可观察时写 `not_observable`。不可观察时不得补入人物，不得依据相邻段、身份 reference 或叙事猜测生成身体部分；所有可观察主人物都必须替换，不能漏人或二选一。
5. 按同一物理环境建立 `scene_plans`，每项作为冻结新场景包；同一场景组件的全部成员段逐段复用同一环境设计、五类变化和 reference，不得逐帧重新设计。每段恰好属于一个场景组件。每个场景方案必须同时包含语义、几何、纵深、布局和局部固有色的真实变化；不得只改色相、材质或全局调色，不得把原场景换皮后冒充新场景。
6. 人物与新场景都保持画幅、裁切、机位、镜头、透视、构图、焦点和景深；固定全局光源方向、曝光、白平衡/CCT、tone curve 和整体色彩风格。新几何可以产生物理正确的局部阴影和反射变化，但不能改变全局照明。
7. 把可见关系图完整写入每段 `protected_relations`。冻结交互、接触、持握、支撑、遮挡、前后顺序、视线、姿态、动作目的、数量、尺度和叙事关系；每帧以当前源帧为几何事实，reference 只提供已声明的身份、场景设计或布局角色。新人物和新场景不得扭曲、融合、穿模或违反现实规则。
...
逐人物验证：目标身份确实改变、源身份不再可识别、冻结新身份包与局部颜色在所有可观察帧逐段一致；`not_observable` 只能为 `not_applicable`，并确认没有新增该人物或身体部分。

逐场景验证：环境语义、几何、纵深、布局和局部固有色都真实改变，不能只有调色或纹理变化；同一场景组件必须逐段符合冻结新场景包。验证全局光源方向、曝光、WB/CCT、tone curve 和整体色彩风格保持，允许新几何产生合理局部阴影。验证交互、接触、持握、支撑、遮挡、前后顺序、视线、姿态、动作目的、相机与构图，检查段内连续性、跨段人物身份与场景组件连续性、漏人物、串人和计划外人物。`hard_cut` 两侧不要求场景连续，但必须确认切后设计没有继承无切后证据的切前场景特征。
