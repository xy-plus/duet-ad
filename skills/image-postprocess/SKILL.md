...
所有冻结图都是源证据。图1始终是唯一编辑画布；源 `reference` 只作负样本证据，不是 target reference：它用于定位必须消失的旧身份/旧场景和必须保持的布局关系，不得向结果复制旧设计。只有生成并冻结且逐项通过资格检查的新 pack 才能作为目标身份或目标场景 reference；不得使用首帧内容锚定其他帧。
...
3. 为每个稳定主人物建立一个 `person_plans` 项，将 `replacement_identity/wardrobe_change/local_color_change` 写成可独立生成、跨帧复用且明显区别于源人物的 `new identity pack` 规格。保持源人物可见的人口外观范围和叙事气质，但不推断敏感身份；新脸、发型轮廓与服装设计不得近似复制源人物。
4. 每段 `persons` 必须按 ID 完整枚举所有主人物。人物在本段任一帧可观察且边界可定位时写 `replace`，并列出所有可观察帧；确实不可观察时写 `not_observable`，不得新增人物。只有每个可观察主人物都有唯一、可生成、可冻结的新身份规格时才合格；不能漏人、合并多人或二选一。
5. 按同一物理环境建立 `scene_plans`，把字段整体作为可独立生成并跨所属段复用的 `new scene pack` 规格；每段恰好属于一个场景组件。每个所属段都必须可见地完成环境语义真实更换、形状/几何与空间结构变化、纵深变化、空间布局变化、局部材质或固有色变化。不得只改色相、材质或全局调色；纯纹理或原结构换皮同样不合格。
...
8. 每个人物和场景组件选一张目标完整、清晰、遮挡少的源帧作为负样本 `reference`；每段选一张最能表达源构图与空间关系的帧作为 `layout_reference_frame_index`。引用只能指向对应人物可观察或场景所属的真实输入帧，不能冒充新身份或新场景 pack。
...
逐人物、逐可观察帧验证：目标身份符合冻结的 `new identity pack`、源身份不再可识别、新身份与局部颜色连续一致；任一可观察主人物仍像源人物、漏换或串人即失败。`not_observable` 只能为 `not_applicable`，并确认没有新增该人物。

逐场景、逐所属段验证：结果符合冻结的 `new scene pack`，旧场景语义与空间骨架不再构成目标设计；环境语义、形状、空间结构、纵深、空间布局及局部材质或固有色都真实改变。任一段仍是原场景改色、纯调色或纯纹理即失败。另验证全局光源方向、曝光、WB/CCT、tone curve，以及接触、遮挡、动作、现实关系和段内/跨段连续性；新几何只可产生合理局部阴影。
