...
ChatGPT 式单页应用（Apple 风格）+ FastAPI 后端：用户上传清洁类参考视频，后台由沙箱化的本地 codex agent 按 `skills/seedance-cleaning-video-maker` 处理，产出 ≤9 张关键帧 + Seedance prompt + 15s 占位预览视频；预留 Seedance 真实提交接口（默认 501 未开放）。
...
- 改 API 响应字段先读 `docs/agent/reference/reference.md`：detail 是冻结的 14 字段契约，meta.json 内部字段（submitted_at/task_id 等）不外泄。
