...
- 4fps 关键帧由 ffmpeg 按 `v:0` PTS 顺序批量解码，VFR 视频不依赖 OpenCV 毫秒随机 seek。新的多模态 H3 链路保留每个子片段联合生成的音轨，把音视频共同按 source EDL 的整数帧预算归一为 24fps 后拼接，不回挂源音轨；源音频只作为上游 H3 素材。协调状态单独持久化 exact `H3Result.attempt_id`，拼接只按该 ID 加载对应 `attempt.json`/media timeline 并冻结 receipt SHA，禁止用业务重试次数或 `latest` 猜测。联合音画模式下 `continue` 不单独删除视觉帧；静音/历史链路也只有本地解码 SHA 明确证明边界重复才允许删除。receipt v2 冻结 FFmpeg 版本、PTS/DTS、源/供应商/最终音频 SHA 和 dialogue/action anchors，且不使用 `aresample async` 拉伸掩盖错位。拼接失败只重跑本地拼接。
