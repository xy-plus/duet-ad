...
开发前先读 [web-next 约束](web-next/AGENTS.md)。组件只能从 [`src/ui/antd.ts`](web-next/src/ui/antd.ts) 导入，Token 与 Provider 只在 [`src/ui/theme.ts`](web-next/src/ui/theme.ts) 定义；ESLint、Stylelint 和 [`governance.test.ts`](web-next/src/governance.test.ts) 是不可绕过的治理入口。
