...
def extract_audio(cdir: Path) -> Path | None:
    ...
    # 探测不了（缺 ffprobe/出错）不拦截，交给 ffmpeg 裁决。
    ...
    if ffprobe:
        try:
            ...
            if r.returncode == 0 and not json.loads(r.stdout or "{}").get("streams"):
                return None
...
def validate_voice_lines(raw: bytes, duration_s: float) -> list[dict]:
    ...
    校验：UTF-8 可解、JSON 解析为非空 list、每项含非空 text 与 number 型
    start_s/end_s 且 0 ≤ start_s < end_s ≤ duration_s（边界允许 0.01s 浮点误差）、
    start_s 单调不减。错误信息指明第几项与原因；返回项只保留白名单三字段。
    """
    from app.pipeline import PipelineError  # 循环导入：pipeline 顶层导入本模块
    ...
    for i, item in enumerate(data):
        ...
        prev_start = start_s
        lines.append({"text": line_text.strip(), "start_s": start_s, "end_s": end_s})
    return lines
