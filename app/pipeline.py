...
def _recover_long_plan(cdir: Path, meta: dict) -> dict:
    ...
    try:
        ...
        if (
            isinstance(meta.get("source_width"), int)
            and isinstance(meta.get("source_height"), int)
        ):
            profiles, aspect_ratio, resolution, fit_mode = _generation_defaults(
                anchors, meta
            )
        else:
            profiles, aspect_ratio, resolution, fit_mode = (
                _legacy_generation_defaults(anchors)
            )
    ...
    try:
        ...
        if not segments:
            ...
            if new_input_contract:
                ...
                profiles, aspect_ratio, resolution, default_fit_mode = (
                    _generation_defaults(anchors, meta)
                )
                changes.update(
                    fit_required=profiles[aspect_ratio]["fit_required"],
                    fit_profiles=profiles,
