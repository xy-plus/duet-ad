...
- H3 生成参数 UI：在台词/画幅适配区暴露 `16:9|9:16` 与 `480p|768p`，服务端按实际 H3 输入帧/源视频预选并冻结，生成后回显画幅、清晰度、台词、适配、时长/分段数；设计已冻结。（→ `feat/h3-generation-params-ui`）
- H3 长视频 10 秒分段：15s planner 选中 10.84s 场景边界后被 `ceil` 为 11s；新计划/提示词/请求须 ≤10s，boundary 原始输出的有界时长偏差由 stitch 精确归一，旧 receipt 仍只读恢复；设计已冻结。（→ `fix/h3-segment-max-10s`）
