...
| `app/stitch.py` | provider-generated A/V EDL、24fps H.264/AAC 归一化、证据式边界去重和 PTS/DTS receipt 门禁 | conversation-task |
...
- `duration_s` 以 `v:0` 实际浮点时长写 receipt；上传、pipeline 重探测和提交门禁限制为 300 秒。短链请求允许到 15 秒；`>15s` 的新长链先把冻结边界归一到六位小数再 `ceil`，每段请求不超过 14 秒。历史 plan v1 保留原始浮点换算只为重建 11–15 秒的已有 boundary attempt，并禁止新 POST。stitch receipt v2 按 source EDL 累计边界换算整数 24fps 半开帧区间，完整解码并验证 PTS/DTS；旧 stitch v1 只读、不复用。新多模态链路用 `provider_generated` 保留并拼接每段 H3 联合生成音轨，只按协调状态持久化的 exact `H3Result.attempt_id` 读取相应 media timeline/receipt SHA，不扫描 `latest`；源音频只作上游素材且不进入最终 remux。`keep` 仅供历史兼容，`aresample async` 不得被当作同步修复。
