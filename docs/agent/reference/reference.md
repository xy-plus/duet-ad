...
- 请求：JSON `{"options": {"change_bg": bool, "face_hold": bool, "remove_subtitle": bool, "remove_brand": bool}, "confirm": true}`；四选项至少一个为真，否则 422；选项值非 bool 同样 422
...
| 4 | `options` 非对象/四选项全假/选项值非 bool | 422 `at least one option required` |
| 5 | `status != "done"` | 409 `artifacts not ready` |
| 6 | `meta.postprocess.status` 已 `running` | 409 `already running` |
| 7 | 每会话锁内复查：meta 消失或已在 running | 409 `already running` |
| 8 | 锁内复查：目标帧目录缺失/为空 | 409 `artifacts not ready` |
| 9 | 后台逐帧执行中任一帧失败 | 受理后 meta 落 `postprocess.status=failed`，`error` 指明帧名（已成功帧保留） |
...
- 后台任务（`postprocess.run_task`，BackgroundTasks，独立路径不吃管道闸；每会话一把锁，可跨会话并发）：
  ...
  - `face_hold`：先 cv2 haarcascade 正面人脸检测（`cv2.data.haarcascades + 'haarcascade_frontalface_default.xml'`，数据文件缺失按无人脸处理），有人脸才注入该选项；无人脸该帧跳过此选项，无适用选项的帧整帧跳过
  ...
  - 任一帧失败 → 整体 failed（`error` 指明帧名，脱敏 ≤300 字）；已成功帧保留；`meta.postprocess.frames` 记有优化版的帧名列表（单段 = 帧名；多段 = `segments/N/帧名`，与 files 白名单路径同形）
...
- `app.postprocess.start(settings, cid, payload, locks) -> dict` — 后处理门控 + 置 `meta.postprocess=running`；返回勾选选项（路由层据此调度后台任务）；失败抛 `PostprocessError(status, detail)`
...
| `postprocess` | dict \| null | 后处理状态（仅后处理启动后存在）：`status`（`running/done/failed`）/`options`（勾选选项）/`frames`（有优化版的帧名列表，单段=帧名、多段=`segments/N/帧名`）/`error`（失败原因，指明帧名）；进 detail 响应 `postprocess` 字段 |
