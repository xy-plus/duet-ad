...
默认长链同一 `chain_id` 严格串行，最多并发两条 chain；每段请求只使用本段冻结参考图。快速长链先预构造并 prepare 全部分段，随后最多 8 个短生命周期 worker fan-out POST，再最多 8 个 worker GET 轮询；队列可以超过 8，但不会创建无界长生命周期线程。单段确定失败只重做该段，已成功下游独立复用；任一子任务 `submission_unknown` 锁住整批且该段不再 POST，快速模式仍让已知兄弟 task 完成 GET。每段 H3 结果的 exact `attempt_id` 独立于业务 `attempt` 落盘；新多模态拼接必须用它加载对应 attempt receipt/media timeline 并冻结 receipt SHA，缺失即关闭，禁止扫描 `latest`。新多模态链路全部成功后，ffmpeg 保留每个 H3 子片段联合生成的音轨，把音视频共同按 source EDL 的累计边界换算整数 24fps 半开帧区间，再归一为 H.264/AAC 并按顺序拼接；源音轨只作为 H3 上游素材，不回挂到最终视频。联合音画模式的 `continue` 不单独删除视觉帧，以免动作相对音频提前；静音和历史链路只有前段尾帧与后段首帧的本地解码 SHA 完全相同，receipt v2 才记录并删除这一帧。receipt 同时绑定算法和 FFmpeg 版本、上游 attempt receipt SHA、源/供应商/最终音频 SHA、各流 start/time_base/首尾 PTS/DTS/单调性、完整解码结果及 dialogue/action anchors。`keep` 只用于历史兼容，`aresample async` 不得被当作同步修复；旧 stitch v1 只供审计且会触发零付费本地重拼。
