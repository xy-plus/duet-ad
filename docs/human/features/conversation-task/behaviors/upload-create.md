...
| 源视频可读且实际时长为正有限数 | ffprobe 校验视频流尺寸；时长超过 10 秒时删除刚创建的会话并返回结构化 422，前端弹窗提示裁剪后重新上传 |
...
| 用户在首次 H3 attempt 前二次修改源提示词 | 以当前 SHA-256 做并发保护；保存后重写并复核 prepared receipt |
| 用户确认台词和画幅后点击“生成最终视频” | 后端冻结本次台词、画幅、关键帧和 H3 源提示词，直接提交 AutoDL H3 |
...
- 10.01 秒视频：上传后返回 `video_duration_exceeds_h3_limit`，不创建可见会话、不运行准备流水线。
