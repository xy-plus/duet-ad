...
| 源视频超过 15 秒 | 创建时只接受 `voice_mode=keep`；准备阶段形成 provider 整秒时长不超过 14 秒的安全分段、首尾锚点和冻结 plan receipt |
...
| 后端提供冻结的 H3 多模态输入 | video-maker 不重新选帧，只输出 H3 多模态语义计划：人物可绑定多张图片和唯一声线参考，人物台词与旁白按同一全局发声顺序保留原语言、原文和标点，环境声与音效只引用已冻结素材 |
...
- 新项目 `≤15s` 是多图参考单段契约；`>15s` 是多图参考分段契约。二者都属于 schema v2，但使用不同冻结 receipt；历史 11–15 秒分段计划保持原契约恢复。
- 多模态计划中的图片和音频引用是稳定的 1-based 外部编号；人物可引用多张图片。旁白不绑定画面人物，台词与旁白共享连续的全局发声顺序。
- 人物、图片、声线之间的声明式关系不是供应商 speaker-face API；事件顺序不是精确 PTS，参考音频也不承诺样本级音频复用。生成后仍须验收人物发声关系、逐字台词和声音事件。
- 媒体字节、哈希、格式、模式和供应商参数由后端冻结与校验，不进入 video-maker 的语义计划。
