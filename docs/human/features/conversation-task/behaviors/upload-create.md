...
| 用户确认台词和画幅后点击“优化 Context IR” | 后端冻结本次台词、画幅、关键帧与基础提示词，只提交 Context IR；此时绝不提交 H3 视频任务 |
| Context IR 完成 | 页面立即显示完整优化提示词；即使 IR 改写了台词而暂不满足约束，也必须显示正文和明确校验状态，不能只显示 `ir_dialogue_mismatch` |
| 用户二次修改 IR 提示词 | 以当前 SHA-256 做并发保护；后端重新校验结构化台词，校验通过才保存为 H3 的唯一输入 |
| 用户点击“生成最终视频” | 只使用已经冻结且可见的 Context IR 提示词推进 H3；没有 IR、IR 尚未完成或正文未通过台词校验时拒绝提交 |
