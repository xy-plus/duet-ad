...
- 选择色相时，项目级替换与每段第一句统一写“只旋转色相”，目标只使用不携带明度、感知亮度、饱和度或表面处理含义的色相类别；环境背景域只允许这种 hue-only 映射。同一输入像素或物理区域的原明度、感知亮度和饱和度不变，不抬亮暗部或压暗高光；局部对比、亮度分布、渐变方向、渐变幅度和原有低频结构逐帧继承，跨帧同一原始明度映射为同一输出明度，不得统一填充为固定亮度。第一句最前面写这些通道不变量再写本图边界，第二句合并全部非目标冻结。
