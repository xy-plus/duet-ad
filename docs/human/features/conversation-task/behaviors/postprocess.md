...
- 对每个实际替换的元素，项目级只冻结一个替换身份，直接复用现有 `PERSON` 或 `SCENE` 映射和项目级短语，不增加字段。身份只包含实际改变的低频属性和必须稳定的可观察特征，其他属性继承各输入帧；跨帧不变量包括类别、目标属性值或区间、表面响应、纹理族与尺度、原有特征相位、结构单元及接缝或重复拓扑，逐帧投影变量仅包括视角、裁切、连续受光、运动模糊和真实遮挡。相机与光照不能解释身份重生；同一物理实例在遮挡后、跨段或 hard cut 后再次出现，仍映射回同一身份和拓扑，不得重新设计或采样高频细节。
- 每帧必须完整覆盖全部可见目标成员，不留旧外观孤岛；目标域外像素硬保留，逐帧保持轮廓、层级、可见性和运动模糊，边界置信不足时保留输入像素。无法从全部冻结帧证明身份和边界可稳定复用时，该段或组件不改动。
- 每段全部关键帧共享一份两句自然语言提示词：第一句合并唯一替换目标、已冻结的项目级身份或短语和本帧投影条件，第二句合并完整覆盖、目标域外硬保留和核心关系保护；保持简洁，不泄漏内部映射标签。`independent_parallel` 只要求当前图服从已冻结身份，不声称看过其他图片；锚帧一致性模式的其他图片只提供目标设计，不能传递人物、构图、动作、物体或关系。两种内部模式都不显示在页面。
- 只有确实被替换且跨至少两段重复的人物或背景进入跨段统一设计。服装、道具、商品和宠物不建立差异化映射。该约束降低误改风险，但不表示供应商输出一定遵循提示词，也不表示 H3 会逐秒复刻原视频镜头节奏。
