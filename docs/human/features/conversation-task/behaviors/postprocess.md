...
- 背景路线唯一允许变化为低频色相时，冻结的 `SCENE replacement` 或短视频项目级身份短语只能由“只改变目标域色相为”与紧随其后的单一朴素色相组成，不得带强度、明暗、饱和度、对比度、材质、光泽或风格修饰；首句最前面逐字写入该短语。每次请求都以当前源图为逐帧基准，保持原明度、原饱和度或色度、原空间渐变与局部对比，不得把不同帧统一成同一亮度或饱和度。
