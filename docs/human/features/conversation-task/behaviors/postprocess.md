...
- 编辑资格门先于提示词：候选 `SCENE` 在目标段的任一冻结帧中，只要全部目标成员或完整外边界不能稳定定位，或目标不可见、仅见边缘碎片或局部窄条，该段就严格输出两句无条件不改动提示词，不得进入该元素，也不得收到共享编辑提示词；不得用同段其他清晰帧补证。只有段内所有冻结帧通过资格门，才在已证明的完整目标域内全覆盖并将目标域外像素硬保留。
