...
- `scene_plans` 对全部段无重叠全覆盖；每个所属段必须同时改变环境语义、可见形状与空间结构、纵深、空间布局和局部材质/固有色。scene boundary 与五维变化逐项相容，新增结构不得越过声明边界；冲突则 `eligible=false/scene_replacement_unsafe` 并输出空计划。纯调色、纯纹理、只换材质或原结构换皮均不算换景。
