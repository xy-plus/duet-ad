...
- 所有项目统一为 `segments[N>=1]`，每段读取和输出 exact 3 个有序关键帧槽位。短 scene 可以在同一 scene 内重复最近合法源帧并绑定 provenance；`N=1` 不选择独立 short 逻辑。
...
- schema、exact-3、索引、普通文件路径和 SHA 属于技术完整性合同；这些字段无效与质量分数低是两类不同问题，不能用备用图片、旧计划或另一 workflow 混淆处理。
