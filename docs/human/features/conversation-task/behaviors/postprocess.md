...
- 三个阶段在每段内严格按“文字/字幕 → Logo/图标 → 图片优化”执行；段之间并行，段内每个阶段的帧并行受服务端并发上限约束。图片优化先逐帧识别核心实体、交互实体、非目标前景与交互支持区域，并按跨段组件冻结一次策略。背景目标只能是一个可追踪的物理背景元素；其编辑域成员在所有目标可见帧中统一纳入或排除，内部几何折转不作停止线，只有通向不同物理元素的外部边界可以停止，边界外保持不变。每个目标可见帧都须独立证明目标每个可见部分的完整原外轮廓、外轮廓转折和遮挡边缘清晰可定位，不得由段内其他帧补证；任一不满足则该段背景候选不合格。背景提示词须沿本图原外轮廓、外轮廓转折和遮挡边缘停止，保持轮廓几何并禁止补全、拉直、平滑、扩张、收缩或改形；仅目标域发生所选属性变化，目标域外保持原颜色与明暗，禁止全局调色、白平衡、曝光、对比度或重新布光。交互支持区域、核心实体、交互实体和非目标前景不得作为背景候选或差异化目标。任一相关段存在高风险交互关系时，组件只允许背景路线；找不到安全背景则不改动，不得回退人物路线。无高风险关系且背景候选整体失败时，只有同一人物在全部相关段稳定清晰且换脸安全才统一仅换脸；两条路线都不安全时生成不改动提示词。
