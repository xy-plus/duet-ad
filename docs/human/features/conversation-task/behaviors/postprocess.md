...
- 背景路线唯一允许变化为低频色相时，冻结的 `SCENE replacement` 或短视频项目级身份短语只能由“只改变目标域色相为”与紧随其后的单一朴素色相组成；色相身份不得附加任何会改变深浅、浓淡、质感或风格的修饰，首句最前面逐字写入该短语，可见差异只由色相本身实现。除色相外，目标域保持当前源图的深浅、浓淡、渐变和局部层次不变；整图中目标域之外的内容保持当前源图不变。
- 编辑资格门先于提示词并适用于每个实际替换目标（`PERSON` 或 `SCENE`）：任一冻结帧的完整目标域或完整外边界不能稳定定位即不合格；候选 `PERSON` 的脸部目标不可见、严重遮挡或严重裁切时不合格，候选 `SCENE` 的目标不可见、仅见边缘碎片或局部窄条时也不合格。不合格时仅该段严格输出两句无条件不改动提示词，不进入对应 `segments`，不得收到共享编辑提示词或用同段其他清晰帧补证，其他合格段继续保留；全帧过滤后长视频同一候选不足两个合格段才不建立该元素。逐段资格过滤不得扩大 no-op 范围，只有跨段同一替换身份本身无法证明时，资格门才把整个组件判为 no-op。只有段内所有冻结帧通过资格门，才在已证明的完整目标域内全覆盖并将目标域外像素硬保留。
- 段级两句自然语言提示词会复用于该段每次以一张当前源图为编辑目标的请求，只发送到所有冻结帧均通过资格门的段，并只写通用投影守恒；不得逐帧描述条件、声称同时看过多张输入图或加入以目标不可见为前提的运行时条件句。`independent_parallel` 只依赖当前源图和冻结身份短语；锚帧一致性模式的其他图片只提供目标设计，不能传递人物、构图、动作、物体或关系。
- 短视频 `[0]` 的 `global_elements` 为空，segment 0 提示词中的自然语言项目身份短语是所有关键帧唯一身份载体；短视频唯一段的任一冻结帧不合格时，segment 0 严格输出两句无条件不改动提示词。长视频只为确实替换且跨至少两段重复的人物或背景建映射，每个 `replacement` 逐字出现在所列各段提示词第一句；实际替换元素的 `segments` 两两不相交，同段有多个候选时按既有安全门和冻结策略只保留一个。全帧资格过滤后不足两个合格段则不建立该元素。服装、道具、商品和宠物不建立差异化映射。该约束不表示供应商输出一定遵循提示词，也不表示 H3 会逐秒复刻原视频镜头节奏。
