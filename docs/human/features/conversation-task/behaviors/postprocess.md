...
- 视觉关键帧冻结后，隔离 Codex 只执行一次 `image-postprocess` Skill：短视频读取逻辑段 `0`，多段项目读取全部分段关键帧；一次生成跨段重复人物、服装、场景、道具和商品的统一映射，以及每段真实提交给 Seedream 的提示词。Skill 不读取或复制视频生成提示词；页面仍只展示每段提示词。
