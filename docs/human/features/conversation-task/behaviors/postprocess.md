...
| `postprocess.status == "running"` | 结果区（最终视频之前）以聊天消息展示：用户气泡（勾选项摘要 chips）+ 助手「后处理进行中…」状态卡，随轮询更新；入口消息隐藏 |
...
- 输入：打开一个 `done` 会话（接口已开放）→ 结果区末尾入口消息点「是」→ 取消其余勾选只留「换背景」→ 确认 → 输出：弹窗关闭，聊天消息出现「后处理进行中…」卡，完成后「优化后」网格
