...
- 选择色相时只使用 CIELAB LCh：项目级冻结一个与源主色相具有可辨别圆周角差的整数目标角，统一执行“仅将 h° 旋转到 h°=N，逐像素 L*=原值、C*=原值”，实际输出以该整数替换 N。目标角不附带颜色名称或其他外观修饰；环境背景域只允许此操作。`SCENE replacement` 与每段第一句逐字复用同一操作短语；第一句按操作、编辑域、边界顺序，第二句合并非目标实体与关系保护及全局调色、重新布光、摄影增强禁止。
