...
- 视觉关键帧冻结后，多段项目先在隔离工作区执行一次 `image-continuity` Skill，检查全部分段并冻结重复人物、服装、场景、道具和商品的统一元素映射；映射只描述身份和新设计，不含构图、机位、动作或位置。随后各段并行执行 `image-postprocess`，Codex 只看到本段关键帧、编辑模式和后端筛选出的本段映射。短视频不增加全局分析。两类 Skill 都不读取或复制视频生成提示词；页面仍只展示每段真实 Seedream 提示词。
