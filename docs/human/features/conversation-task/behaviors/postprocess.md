...
- 视觉关键帧冻结后，隔离 Codex 只执行一次 `image-postprocess` Skill：短视频读取逻辑段 `0`，多段项目读取全部分段关键帧；每段只选择人物外观或背景中的一个主要替换目标。多帧足以稳定识别人物脸部和服装时优先人物，否则选择背景；选择人物时保持背景，选择背景时保持人物和服装。Skill 不读取或复制视频生成提示词；页面仍只展示每段提示词。
...
- 三个阶段在每段内严格按“文字/字幕 → Logo/图标 → 图片优化”执行；段之间并行，段内每个阶段的帧并行受服务端并发上限约束。图片优化提示词以目标设计开头，按 Unicode 字符计不超过 300 个字符；超长会重写而不是截断。功能道具、玩具、核心商品、手部，以及人与物或物与物的握持、接触、插入、对齐、连接、遮挡、前后顺序、数量、结构和方向均被要求保持；提示词只点名最易误改的核心物体和关系，不复述全部构图。锚帧一致性模式的其他图片只提供所选人物身份或背景设计参考，不能把构图、动作、物体或关系传给当前目标图；两种内部模式都不显示在页面。
- 只有确实被替换且跨段重复的人物、配套服装或背景会形成跨段统一设计，相同替换描述逐字复用；道具、商品和宠物不作为新的差异化映射。该约束用于降低误改风险，不表示供应商生成结果在机制上绝对不会偏离提示词。
