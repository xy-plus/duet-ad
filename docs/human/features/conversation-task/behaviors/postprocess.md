...
- 每个实际替换元素在项目级只有一个身份：`PERSON` 只冻结同一新脸身份，非脸部继承当前源图；`SCENE` 才冻结表面局部物理坐标中的实际目标属性、材料、表面响应、纹理族与世界尺度、原有特征相位、结构单元及接缝或重复拓扑。视角、裁切、连续受光、运动模糊和真实遮挡仅服从通用投影守恒，不能重生身份；同一物理实例在遮挡后、跨段或 hard cut 后再次出现仍映射回同一身份。
- 编辑资格门先于提示词：任一目标可见帧的全部目标成员或完整边界不能稳定定位，该段或组件输出两句不改动提示词。只有所有目标可见帧通过资格门，才在已证明的完整目标域内全覆盖并将目标域外像素硬保留；目标完全不可见帧不改动，不再用边界局部保留折中。
- 段级两句自然语言提示词会复用于该段每次单图请求，只写对每次当前待编辑源图都成立的通用投影守恒，不逐帧描述条件，也不声称同时看过多张输入图。`independent_parallel` 只依赖当前源图和冻结身份短语；锚帧一致性模式的其他图片只提供目标设计，不能传递人物、构图、动作、物体或关系。
- 短视频 `[0]` 的 `global_elements` 为空，segment 0 提示词中的自然语言项目身份短语是所有关键帧唯一身份载体，不声称存在 `PERSON` 或 `SCENE` 映射。长视频只为确实替换且跨至少两段重复的人物或背景建映射，每个 `replacement` 本身是可直接复用的自然语言冻结短语，并逐字出现在所列各段提示词第一句；服装、道具、商品和宠物不建立差异化映射。该约束不表示供应商输出一定遵循提示词，也不表示 H3 会逐秒复刻原视频镜头节奏。
