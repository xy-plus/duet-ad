...
- 三个阶段在每段内严格按“文字/字幕 → Logo/图标 → 图片优化”执行；段之间并行，段内每个阶段的帧并行受服务端并发上限约束。高风险手—功能道具关系存在时，图片优化优先只原位替换一个跨段稳定的大面积背景表面，优先地面、其次墙面；保持边界、拼缝、方向、透视、接触几何、光线和固定构件。只有没有高风险关系且找不到安全表面时，才允许只换同一人物的新脸；不换服装。两条路线都不能证明安全时生成不改动提示词。功能道具、玩具、商品、手部及其物理关系始终不可编辑。
