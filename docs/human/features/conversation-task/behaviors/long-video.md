...
| 当 | 则 |
| --- | --- |
| 原视频完成分析 | `video-maker` 规划连续 `segments[N>=1]`；每段固定冻结 9 张原始关键帧、视频提示词、动作/镜头/时间关系 |
...
| 所有分段成功 | 使用同一拼接器按顺序处理 H3 原生音视频；`N=1` 的输入列表长度为一，不调用另一个短片拼接器 |
...
- 输入：14.5 秒视频，规划为一个 segment → `segments.length=1`，执行一次统一 H3 segment 任务，再由统一拼接器处理单元素列表。
