...
| `generated.mp4` 存在且 Context IR 可用 | 最终视频下仍保留“Context IR 优化提示词”折叠区，可按需查看 |
...
- Context IR 正文只经带 Bearer 鉴权的只读按需 API 获取；详情轮询不得包含正文、provider task id、result URL、凭据或其他供应商原始数据。
