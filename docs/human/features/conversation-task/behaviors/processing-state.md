...
| 选了口播转换时：抽帧后先做口播听写（codex 听 work/voice.mp3，按模式保持/洗稿/翻译，输出带时间戳台词） | 台词经 YAMNet 声学验证（只保留真有口播声的句子，纯 BGM/唱歌的假转录丢弃）后随结果落盘（进 meta.voice_lines，detail 响应含 voice_lines 字段）；失败则整个会话 `failed` |
