...
| 选了口播转换时：抽帧后先做口播听写（codex 听 work/voice.mp3，按模式保持/洗稿/翻译，输出带时间戳台词） | 台词校验后随结果落盘（进 meta 内部字段，不进 detail 响应）；失败则整个会话 `failed` |
| 任一步骤失败（含 codex 超时 1800s、产物校验不过、抽帧失败） | 状态转 `failed`，`error` 展示截断后的可读原因（≤500 字） |
