...
| 视频 >20 秒：抽帧后先做场景检测（scenes.json），按场景边界拆成 4~15 秒的段，各段独立切视频 + 抽帧 + codex 并行处理 | 逐段产物聚合进 meta.segments（每段关键帧、提示词、该段台词）；台词按句子时间落入段区间归属，该段提示词由后端加「不要生成背景音乐」一行 |
| 视频 ≤20 秒或场景检测不出切点 | 按单段流程处理（不拆段，现有行为不变） |
