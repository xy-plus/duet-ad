...
运营/创作者上传一支最长 300 秒的参考视频，评审系统准备关键帧、分段提示词与自动台词，再显式选择台词和画幅策略，调用视频生成服务并拼接最终视频。15 秒及以内使用单任务链路。
...
- [x] 上传后以首个视频流的视觉时长校验；音频/容器尾巴不影响 15/300 秒门禁，超过 300 秒时拒绝，超过 15 秒按镜头拆成 provider 整秒时长不超过 14 秒的连续片段
