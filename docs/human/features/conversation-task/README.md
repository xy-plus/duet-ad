...
运营/创作者上传一支最长 10 秒的参考视频，评审系统准备关键帧、视觉提示词与自动台词，再显式选择台词和 9:16 画幅策略，直接提交 AutoDL H3 生成最终视频。
...
- [x] 上传后校验实际时长；超过 H3 上限 10 秒时弹窗提示并拒绝入队
...
- 当前新契约把完整源视频冻结为一次生成请求，不截断、不隐式分段；因此在上传阶段严格限制为 10 秒。
