...
运营/创作者上传一支参考视频，评审系统准备关键帧、视觉提示词与自动台词，再显式选择台词和 9:16 画幅策略，直接提交 AutoDL H3 生成最终视频。项目不限制参考视频总时长。
...
- [x] 界面展示实际时长；引擎时长按实际时长向上取整，项目不设总时长上限
...
- 当前新契约把完整源视频冻结为一次生成请求，不因总时长截断；实际可生成时长仍取决于上游工作流能力。
