...
原视频
  ...
  -> video-maker Skill 以优化图为视觉事实重新生成最终视频提示词
  -> Context IR 只优化最终提示词
...
- 全链只允许调用两个 Skill：`video-maker` 负责关键词/片段分析、每段 9 张关键帧和视频提示词，并在图片确认后以优化图为视觉事实重新生成最终视频提示词；`image-postprocess` 只负责关键帧图片优化。同一个 `video-maker` 可以按上述两个既有职责调用两次，但不得派生音频、Binding 或 speaker-visibility phase。
- 已验收的 `image-postprocess` Skill 立即冻结，不再修改其文件、提示词、规则或测试；后续工作只允许拼接已有链路。
- 不存在 Binding Skill、Audio Skill、Speaker Skill 或任何第三个 Skill。音频呈现由普通后端代码根据冻结台词、Web 的画内/画外选择和现有 voice reference 做确定性投影。
...
- 第二次 `video-maker` 输出的新人物、新场景、新对象、服装、材质和空间结构以 9 张优化图为准；动作、镜头、构图、节奏和 segment 时间轴以原视频冻结事实为准。H3 禁止消费第一次调用中的旧视觉 prompt。
- Context IR 只优化第二次 `video-maker` 提示词的文字表达，不得恢复旧视觉元素，也不得改变关键帧顺序、台词、时间、画内/画外选择或 voice reference；IR 完成后直接进入 H3，中间不得增加新阶段。
...
4. 是否保持图片 Skill 字节不变？
5. 图片确认后是否由同一个 `video-maker` 基于优化图重新生成视频提示词，并禁止旧视觉 prompt 进入 H3？
...
运营/创作者上传一支最长 300 秒的参考视频，系统用唯一分段链准备每段 9 张关键帧、视频提示词与自动台词；用户确认优化图并显式选择台词、声音呈现和画面参数后，同一个 `video-maker` 根据新视觉元素重写最终视频提示词，Context IR 优化后直接交给 H3 生成各段原生音视频，最终由同一拼接流程输出新视频。
...
- [x] 每段第二次 `video-maker` 生成的最终提示词对用户可见，并与对应 9 张优化图一起由统一 plan receipt 冻结
