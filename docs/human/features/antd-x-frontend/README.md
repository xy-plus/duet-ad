...
用 Ant Design 与 Ant Design X 重新实现完整会话式 H3 工作台。现行 3211/本机 3212 保留旧 UI；HTTPS 3213 提供新 UI，并通过同源 `/api` 共享同一个 3212 后端和会话数据。
...
- [x] 现有登录、创建、分析、提示词、短长视频生成、恢复/重试、媒体和后处理功能完整保留
- [x] 所有可见控件和表面由 AntD/AntD X 提供，仅原生视频控件例外
...
- [x] 生产只读 smoke 已通过，本 feature 和对应 Agent 文档状态为 `done`
...
- 3213 已由现有 Caddy unit 提供 HTTPS 静态前端与同源 `/api` 反代；旧 Vite preview 不再运行。
