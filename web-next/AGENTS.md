...
- 功能 CSS 只能处理页面几何、滚动、响应式断点和视频宽高比，并使用 Ant Token CSS 变量。`src/app/**` 与 `src/features/**` 禁止声明背景、边框、圆角、阴影、字体、颜色或第二套 CSS 变量；需要视觉表面时必须直接组合 Card、Collapse、Alert、Descriptions、Divider 等官方组件。
- 禁止 `.ant-*` 内部选择器、原始颜色、`!important`、渐变、玻璃拟态和组件式 CSS 类。

## 页面骨架与密度

- 这是会话式 AI 工作台，不是后台管理页。桌面固定为 272px 会话侧栏、固定顶栏和不超过 900px 的居中内容流；页面高只能是 `100dvh`，只允许内容区和会话列表滚动。不得新增超过 56.25rem 的内容壳或用大于 `100dvh` 的高度恢复文档级长页。
...
- AntD 完整 JSDOM 用例统一使用 `vite.config.ts` 的 10 秒预算；ESLint 会拒绝 `it/test/it.each` 的数字超时参数和 `{ timeout }` 选项。撞线时先减少无关交互或拆小 fixture，不能用更长 timeout 掩盖阻塞。
